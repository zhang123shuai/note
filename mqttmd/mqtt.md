# 一、TCP协议-前端vue连接mqtt接收数据（多个mqtt）

## 1.全局引入mqtt地址端口

①.pubilc文件夹-->static文件夹-->index.js

```js
let mqttIP1 = '9.168.0.201';
let mqttD1=61614;
```

②.pubilc文件夹-->index.html

```js
  <script src="./static/paho-mqtt.js" type="text/javascript"></script>
...
```html
<!-- 使用组件的方式 --> 
<mq :params="params" @dataChange="dataChange" />
<!-- 同一页面引入多个的时候 --> 
<mq :params="params1" @dataChange="dataChange1" />
```
  ...
  data() {
    return {
      params: {
        mqttIP: mqttIP1,
        mqttD: mqttD1,
        subscribeName: "car_json",//监听的topic
        userId: "name" + Math.random() * 5000,//多个mqtt服务，id不能唯一
      },
      params1: {
        mqttIP: mqttIP1,
        mqttD: mqttD1,
        subscribeName: "car_json3",
        userId: "name" + Math.random() * 5000,
      },
    };
  },
  created() {},
...
export default {
  ...
  methods: {
    mqttMSG() {
      // 创建一个客户端实例 Create a client instance
      let client;
      client = new Paho.MQTT.Client(
        this.params.mqttIP,
        Number(this.params.mqttD),
        this.params.userId
      );

      // 设置回调处理程序 set callback handlers
      client.onConnectionLost = onConnectionLost;
      client.onMessageArrived = this.onMessageArrived;
      // 连接客户端 connect the client
      client.connect({
        // 当客户端连接时调用 called when the client connects
        onSuccess: () => {
          let message;
          // 建立连接后，订阅并发送消息 Once a connection has been made, make a subscription and send a message.
          console.log("onConnect");
          client.subscribe(this.params.subscribeName);
          message = new Paho.MQTT.Message("Hello");
          message.destinationName = this.params.subscribeName;
          client.send(message);
        },
        userName: "clay",
        password: "11223344",
      });
      // 当客户端失去连接时调用 called when the client loses its connection
      function onConnectionLost(responseObject) {
        if (responseObject.errorCode !== 0) {
          console.log("onConnectionLost:" + responseObject.errorMessage);
        }
      }
      // 收到信息时呼叫 called when a message arrives
    },
    onMessageArrived(message) {
      this.$emit("dataChange", message);
    },
  },
};
</script>
...
```js
export const MQ_SERVICE = 'ws://192.168.120.156:61614/stomp' // mq服务地址
...
import { MQ_SERVICE, MQ_USERNAME, MQ_PASSWORD } from "./linkparam.js";
export default {
  name: "entry",
  data() {
    return {
  ...
  methods: {
    sendMessage() {
      axios({
        method: "get",
        url: "http://192.168.120.156:9001/send",
      }).then((response) => {
        //这里使用了ES6的语法
    ...
    responseCallback: function (frame) {
      console.log("responseCallback msg=>3" + frame.body);
      console.log("------");
    },
    connect: function () {
      var headers = {
